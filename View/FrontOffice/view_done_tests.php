...
// SQL query to fetch done tests
$query = "SELECT * FROM reponse r JOIN evaluation e ON r.id = e.id WHERE r.iduser = :user_id AND e.id_chapitre = :id_chapitre";

$done_tests = $evalc->fetchDoneTests($user_id, $id_chapitre, $query);
?>
...
.search-bar {
  ...
  <body>
    ...
    <main id="Evaluation" style="padding: 40px 20px; text-align: center">
      ...
      <?php if (count($done_tests) > 0): ?>
          <?php foreach ($done_tests as $eval): ?>
            <p>
              <a href="#" style="text-decoration: none; color: #007bff; font-size: 18px;">
                Test ID: <?php echo $eval['id']; ?> - <?php echo $eval['matiere']; ?>
              </a>

              <?php if (!$eval['note']): ?>
                <!-- If no note is set, show modify and delete options -->
                <a href="modifierReponse.php?id_reponse=<?php echo $eval['idrep']; ?>"
                   style="text-decoration: none; color: #d9ff00; font-size: 18px;">Modifier</a>
                <a href="supprimerReponse.php?id_reponse=<?php echo $eval['idrep']; ?>"
                   style="text-decoration: none; color: #ff0015; font-size: 18px;">Supprimer</a>
              <?php else: ?>
                <!-- If note is set, display note and remark -->
                <span> - Note: <?php echo $eval['note']; ?> - Remarque: <?php echo $eval['remarque']; ?></span>
              <?php endif; ?>
            </p>
          <?php endforeach; ?>
        <?php else: ?>
          <p>No tests marked as done.</p>
        <?php endif; ?>
      </div>
    </main>
    <!-- Test Links End -->
